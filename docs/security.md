...
문제가 생겨도 전체 애플리케이션 디렉터리에 무조건 `777`을 적용하지 않음. 쓰기가 필요한 경로만 제한적으로 조정함.

Docker Compose의 PHP 컨테이너는 루트 `.env`의 `APP_UID:APP_GID` 값으로 실행됨. 여러 개발자가 같은 저장소를 사용할 때는 각자 `id -u`, `id -g` 결과를 루트 `.env`에 설정해 호스트 파일 소유권과 컨테이너 실행 사용자를 맞춤.

## 업로드와 스토리지

파일 업로드는 PHP와 Nginx 양쪽 제한을 함께 봐야 함.
